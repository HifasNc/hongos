    ...
    <div className="relative" ref={wrapperRef}>
      ...
      <div className="relative">
        <input
          ...
          required={required}
          autoComplete="off"
          className="w-full px-3 py-2 bg-[#f2a641] text-[#040214] border border-stone-300 rounded-md shadow-sm placeholder-[#040214]/60 focus:outline-none focus:ring-2 focus:ring-emerald-500 sm:text-sm pr-10"
        />
        <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
          <ChevronDownIcon className="h-5 w-5 text-[#040214]/50" />
        </div>
      </div>
      {showSuggestions && filteredSuggestions.length > 0 && (
        <ul className="absolute z-[100] w-full bg-white border border-stone-300 rounded-md mt-1 max-h-60 overflow-y-auto shadow-2xl">
          {filteredSuggestions.map((suggestion, index) => (
            <li
              key={suggestion}
              onClick={() => handleClick(suggestion)}
              className={`px-3 py-3 cursor-pointer border-b border-stone-100 last:border-0 hover:bg-emerald-50 font-medium !text-[#040214] ${
